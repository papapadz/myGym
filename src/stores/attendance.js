...
export const attendanceStore = defineStore('attendance', {
    ...
    actions: {
      ...
      async fetchAttendanceDataByYear(ddate,filterBy,dept) {
        console.log(ddate)
        try {
          const response = await axios.get(`${configStore().getServerURL}/api/mobile`+'/attendance/data', {
            params: {
              date: ddate,
              flag: filterBy,
              dept: dept!=null ? dept['id'] : ''
            }
          })
          let addlLabel = ''
          if(dept!=null)
            addlLabel = ' of the ' + dept['name']
          
          if(filterBy=='year') {
            this.chartData.datasets[0].label = 'Data for CY ' + format(new Date(ddate),'yyyy') + addlLabel
            this.chartData.labels = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
          } else {
            this.chartData.datasets[0].label = 'Data for ' + format(new Date(ddate),'PPPP') + addlLabel
